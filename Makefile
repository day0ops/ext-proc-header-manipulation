...
NAME ?= ext-proc-header-manipulation
VERSION ?= $(shell git describe --tags --always --dirty 2> /dev/null || echo v0)
LD_FLAGS ?= -s -w \
	-X 'github.com/day0ops/ext-proc-header-manipulation/pkg/version.Name=${NAME}' \
	-X 'github.com/day0ops/ext-proc-header-manipulation/pkg/version.Version=${VERSION}' \
	-X 'github.com/day0ops/ext-proc-header-manipulation/pkg/version.GitCommit=${GIT_COMMIT}'
